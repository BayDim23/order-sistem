...
def init_db():
    conn = sqlite3.connect('busness_orders.db')
    cur = conn.cursor()
    cur.execute('''
    ...
    conn.close()
def add_order():
    con = sqlite3.connect('busness_orders.db')

    cur = con.cursor()
    cur.execute("INSERT INTO orders(customer_name, order_details, status) VALUES (?, ?, 'Новый')",
                (customer_name.get(), order_details_entry.get()))
...
def complete_order():
    ...
    if not selected_item:
        ...
        messagebox.showwarning("Предупреждение", "Выбирите  заказ для завершения")
